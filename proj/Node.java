...
public class Node {
    ...
    case Protocol.TRANSPORT_PKT:
        this.tcpMan.receiveTransportPkt(packet, from);
        break;

    //add new transport protocol
    //give it to TCP manager

    //you can add new protocols like quic here
    ...
    public void addTCPTimer(long deltaT, TCPSock sock, TCPSockTask task) {
        ...
        // String[] paramTypes = {"java.lang.Object"};
        try {
            String[] paramTypes = {"TCPSockTask"};
            Object[] params = {task};
            Method method = Callback.getMethod("handleTimeout", sock, paramTypes);
            Callback cb = new Callback(method, sock, params);
            this.manager.addTimer(this.addr, deltaT, cb);
        } catch(Exception e) {
            logError("Failed to add timer callback. Method Name: " + "handleTimeout" +
                 "\nException: " + e);
        }
    }
